...
Coordinates:
  * y            (y) float64 11kB 3.799e+06 3.799e+06 ... 3.757e+06
  * x            (x) float64 22kB 3.344e+05 3.344e+05 ... 4.166e+05
...
NaN values can occur at the edges of satellite scenes or in areas with clouds. Filling them with zero prevents visualization errors.
...
This initial attempt produces a blank-looking image with warnings about clipping RGB data and invalid values. The issue is extreme outliers (often from cloud reflectance) that make other pixel values appear near-zero when matplotlib scales the image.
...
The EJI dataset contains census tract-level data for all of California. Let's explore its structure:
...
print(eji_data.head())
print(f"\nTotal census tracts: {len(eji_data)}")
print(f"Number of variables: {len(eji_data.columns)}")
```

The dataset contains 174 columns including demographic, environmental, and health vulnerability indicators for each census tract.
:::
