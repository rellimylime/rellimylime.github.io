...
# Both datasets use EPSG:3857 (Web Mercator)
...
# Restore CRS information from spatial_ref variable
...
The dataset contains five spectral bands spanning visible light through infrared wavelengths, with dimensions of 1418 × 2742 pixels covering the fire-affected region.
...
# Create true color composite (RGB)
landsat[['red', 'green', 'blue']].to_array().plot.imshow(robust=True)
plt.title('True Color Image - Landsat 8')
plt.show()
```

The `robust=True` parameter clips extreme outlier values (often from clouds or water) to improve visualization by preventing a few bright pixels from washing out the entire image.
...
This false color composite clearly reveals the extent and severity of fire impacts. The distinct reddish-brown burn scars contrast sharply with the bright green healthy vegetation in surrounding areas. The combination of spectral analysis and geographic boundaries provides a powerful tool for assessing wildfire damage and monitoring landscape recovery.
