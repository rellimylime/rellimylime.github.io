...
<details>
  <summary>Click to hide/show output</summary>
  ```{python}
  #| echo: false
  #| eval: true
...
**Summary:** The dimensions (height and width) of the data are represented by **y: 1418** and **x: 2742**. The data is three-dimensional, with **y, x, and time** as coordinates. The variables in the data are **'red', 'green', 'blue', 'nir08', 'swir22', 'spatial_ref'**.
...
# Check if CRS is properly set
print(landsat.rio.crs)
```

**Output:** `None`

The dataset is not currently geospatially referenced. We need to recover this information from the `spatial_ref` variable.

```{python}
#| eval: false

# Print the CRS from spatial_ref metadata
...
# Verify the CRS is now properly set
print(landsat.rio.crs)
```

**Output:** `EPSG:32611` (WGS 84 / UTM zone 11N)

The Landsat data uses UTM Zone 11N, which is appropriate for the Los Angeles area.
...
```
Bands with NaN values: ['red', 'green', 'blue', 'nir08', 'swir22']
```
...
**First attempt (without outlier handling):**

```{python}
#| eval: true

# Plot the true color image without robust parameter
landsat[['red', 'green', 'blue']].to_array().plot.imshow()
plt.show()
```

This initial attempt produces a blank-looking image with warnings about clipping RGB data and invalid values:

```
Clipping input data to the valid range for imshow with RGB data ([0..1] for floats or [0..255] for integers).
```

The issue is extreme outliers (often from cloud reflectance) that make other pixel values appear near-zero when matplotlib scales the image.
...
**Why `robust=True` Works:**

The `robust=True` parameter clips extreme outlier values (2nd and 98th percentiles) to improve visualization. This prevents a few bright pixels from washing out the entire image, resulting in a clear, detailed true color image with proper contrast and no warnings.
...
The EJI dataset contains census tract-level data for all of California with 174 columns including demographic, environmental, and health vulnerability indicators for each census tract.
...
print(f"Fire perimeters CRS: {palisades_perimeters.crs}")
print(f"EJI data CRS: {eji_data.crs}")

# Reproject EJI data to match fire perimeter CRS
eji_data = eji_data.to_crs(palisades_perimeters.crs)

print(f"\nEJI data reprojected to: {eji_data.crs}")
```

**Output:**
```
Fire perimeters CRS: EPSG:3857
EJI data CRS: EPSG:3857

EJI data reprojected to: EPSG:3857
```

Both datasets now use the same coordinate reference system, enabling accurate spatial operations.
