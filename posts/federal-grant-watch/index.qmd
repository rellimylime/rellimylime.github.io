...
description: "A beginner's walkthrough of three web scraping and API techniques — REST APIs, ID-based enrichment, and HTML scraping — using the DOGE federal grant termination list as a real-world example."
author: "Emily Miller"
date: "2026-03-12"
categories: [federal policy, data analysis, DOGE, R, web scraping, tutorial]
image: figures/academic_domain_comparison_plot.png
format:
  html:
    theme: ../../styles.scss
    code-fold: true
...
The blue line is DOGE's figure. The red dashed line is the USAspending-derived estimate — total obligation minus total outlay, for grants where we could find a match. The gap between them is large and persistent throughout the year.

### The Discrepancy

```{r discrepancy-plot}
#| echo: false
#| fig-cap: "Monthly discrepancy between DOGE-claimed savings and the USAspending remaining obligation estimate. Positive values = DOGE claims exceed what the grant database shows as remaining."
ggplotly(
    plot_academic_discrepancy(tar_read(academic_monthly_totals), tar_read(policy_events)),
    tooltip = "text"
) |>
    layout(legend = list(orientation = "v", x = 1.01, y = 1))
```

This discrepancy is why cross-referencing sources matters. DOGE reports termination values; USAspending records actual obligations. For multi-year grants, DOGE may count future-year funding that was never guaranteed. The independent comparison is only possible because we pulled both sources.
