...
```bash
cd path/to/forest-data-compilation
pip install streamlit pandas pyvis
streamlit run 05_fia/docs/dashboard/fiadb_dashboard.py
```

These commands are for the separate `forest-data-compilation` repository (not this website repo). Point it at your local FIADB SQLite file in the sidebar. Set `FIADB_DB_PATH` as an environment variable on a server and the path input disappears from the UI.
...
- [GitHub repository](https://github.com/rellimylime/forest-data-compilation)
