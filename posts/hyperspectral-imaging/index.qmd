...
.method-card {
  ...
  border: 1px solid rgba(124, 146, 137, 0.38);
  border-top: 3px solid rgba(47, 111, 103, 0.72);
  border-radius: 0.45rem;
...
.method-card-tag {
  ...
  font-size: 0.76rem;
  font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
  color: rgba(47, 111, 103, 0.82);
  letter-spacing: 0.05em;
  text-transform: uppercase;
}
.method-card p:last-child { margin: 0; font-size: 0.93rem; line-height: 1.5; }
...
.img-placeholder {
  ...
  border: 2px dashed rgba(124, 146, 137, 0.45);
  border-radius: 0.45rem;
  background: rgba(124, 146, 137, 0.05);
  color: rgba(39, 46, 54, 0.5);
  font-size: 0.88rem;
  text-align: center;
  min-height: 7rem;
}
.img-placeholder-icon { font-size: 1.8rem; opacity: 0.35; line-height: 1; }
.img-placeholder strong { color: rgba(39, 46, 54, 0.72); display: block; margin-bottom: 0.2rem; font-size: 0.92rem; }
...
<div class="method-cards">
  <div class="method-card">
    <h4>PCA</h4>
    <p class="method-card-tag">Variance view</p>
    <p>Find the dominant directions of spectral variation in the scene. Best for compression, quality checks, and anomaly screening.</p>
  </div>
  <div class="method-card">
    <h4>Linear Unmixing</h4>
    <p class="method-card-tag">Fraction view</p>
    <p>Given known pure-material spectra, estimate what fraction of each material is present in every pixel. Closest to the classic subpixel mixing problem.</p>
  </div>
  <div class="method-card">
    <h4>NMF</h4>
    <p class="method-card-tag">Parts view</p>
    <p>Learn additive spectral parts directly from the data, without supplying endmembers in advance. Sits between PCA compression and supervised unmixing.</p>
  </div>
</div>
```

| Method | Main question | Easiest way to read the output |
|----|----|----|
| PCA | What varies most across the scene? | Contrast axes plus eigenimages |
| Unmixing | What is this pixel made of? | Material fractions |
| NMF | What additive parts can I learn? | Learned parts plus weight maps |
...
%%| fig-align: center
flowchart LR
    A[Load cube] --> B[Inspect spectra]
    B --> C[Reshape to matrix]
    C --> D{Goal?}
    D -- dominant variance --> E[PCA]
    D -- material fractions --> F[Unmixing]
    D -- learned parts --> G[NMF]
    E --> H[Eigenimages]
    F --> I[Abundance maps]
    G --> J[Component maps]
```
...
```{=html}
<div class="img-placeholder scroll-reveal">
  <div class="img-placeholder-icon">🛰</div>
  <strong>Geographic context — image to add</strong>
  Aerial or satellite overview of the HYDICE Urban scene footprint (~307 × 307 px, 2 m GSD), annotated with labeled land cover zones: vegetation patches, road network, rooftops, shadowed areas.<br><br>
  <em>How to get it:</em> the HYDICE Urban scene is located near Washington, D.C. (Chantilly, VA area — confirm in ERDC metadata). Overlay the scene bounding box on a Google Earth or Mapbox satellite basemap using the corner coordinates from the <code>.hdr</code> file, then export a labeled screenshot. Alternatively, annotate the RGB composite directly in Python with <code>matplotlib</code> patches and text.
</div>
```
...
```{=html}
<figure class="concept-figure scroll-reveal">
  <svg viewBox="0 0 520 195" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Diagram showing a 3D hyperspectral cube being reshaped into a 2D pixel-by-band matrix">
    <!-- Cube: front face -->
    <polygon points="28,68 128,68 128,168 28,168" fill="rgba(47,111,103,0.1)" stroke="rgba(47,111,103,0.65)" stroke-width="1.5"/>
    <!-- Cube: top face -->
    <polygon points="28,68 60,38 160,38 128,68" fill="rgba(47,111,103,0.07)" stroke="rgba(47,111,103,0.65)" stroke-width="1.5"/>
    <!-- Cube: right face -->
    <polygon points="128,68 160,38 160,138 128,168" fill="rgba(47,111,103,0.05)" stroke="rgba(47,111,103,0.65)" stroke-width="1.5"/>
    <!-- Cube labels -->
    <text x="78" y="123" text-anchor="middle" font-size="11" fill="rgba(39,46,54,0.65)">H × W</text>
    <text x="78" y="138" text-anchor="middle" font-size="10" fill="rgba(39,46,54,0.45)">pixels</text>
    <text x="155" y="78" text-anchor="middle" font-size="10" fill="rgba(39,46,54,0.55)" transform="rotate(-30,155,78)">B bands</text>
    <!-- Arrow -->
    <line x1="175" y1="103" x2="225" y2="103" stroke="rgba(39,46,54,0.32)" stroke-width="1.8"/>
    <polygon points="221,98 232,103 221,108" fill="rgba(39,46,54,0.32)"/>
    <text x="203" y="122" text-anchor="middle" font-size="10" fill="rgba(39,46,54,0.45)">reshape</text>
    <!-- Matrix: band header row -->
    <rect x="242" y="40" width="248" height="20" fill="rgba(47,111,103,0.14)" stroke="rgba(47,111,103,0.5)" stroke-width="1"/>
    <text x="366" y="54" text-anchor="middle" font-size="10" fill="rgba(39,46,54,0.7)">B = 210 bands (columns)</text>
    <!-- Matrix: pixel column label -->
    <rect x="242" y="60" width="20" height="115" fill="rgba(47,111,103,0.1)" stroke="rgba(47,111,103,0.5)" stroke-width="1"/>
    <text x="252" y="121" text-anchor="middle" font-size="10" fill="rgba(39,46,54,0.65)" transform="rotate(-90,252,121)">N = 94,249 pixels (rows)</text>
    <!-- Matrix: data area -->
    <rect x="262" y="60" width="228" height="115" fill="rgba(250,248,244,0.85)" stroke="rgba(124,146,137,0.45)" stroke-width="1"/>
    <!-- Grid lines (cols) -->
    <line x1="319" y1="60" x2="319" y2="175" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <line x1="376" y1="60" x2="376" y2="175" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <line x1="433" y1="60" x2="433" y2="175" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <!-- Grid lines (rows) -->
    <line x1="262" y1="89" x2="490" y2="89" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <line x1="262" y1="118" x2="490" y2="118" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <line x1="262" y1="147" x2="490" y2="147" stroke="rgba(124,146,137,0.28)" stroke-width="0.8"/>
    <!-- Highlight: first row = one pixel's spectrum -->
    <rect x="262" y="60" width="228" height="29" fill="rgba(47,111,103,0.13)" stroke="none"/>
    <text x="376" y="78" text-anchor="middle" font-size="10" fill="rgba(47,111,103,0.88)">← one pixel's full spectrum →</text>
    <!-- Bottom labels -->
    <text x="94" y="188" text-anchor="middle" font-size="11" fill="rgba(39,46,54,0.6)" font-weight="600">Cube (B × H × W)</text>
    <text x="366" y="188" text-anchor="middle" font-size="11" fill="rgba(39,46,54,0.6)" font-weight="600">Matrix (N × B)</text>
  </svg>
  <figcaption>Reshaping the cube: every pixel's position (row, col) maps to a single row in the matrix, carrying its full 210-band spectrum. The spectral data is unchanged — only the bookkeeping shape.</figcaption>
</figure>
```
...
<div class="img-placeholder scroll-reveal">
  <div class="img-placeholder-icon">📚</div>
  <strong>Spectral library comparison — image to add</strong>
  Side-by-side of hand-picked endmember spectra (this scene) vs. USGS or ASTER library references for the same material classes.<br><br>
  <em>How to get it:</em> download the USGS Spectral Library v7 from <code>https://crustal.usgs.gov/speclab/QueryAll07a.php</code> and query asphalt, grass, and roofing material entries. Export as CSV and overlay with <code>matplotlib</code> against the endmember spectra already saved in this notebook.
</div>
```
